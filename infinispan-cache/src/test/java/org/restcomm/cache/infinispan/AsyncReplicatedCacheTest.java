...
public class AsyncReplicatedCacheTest 
{
	...
	public static void initCluster()
	{
		...
		factory=new InfinispanCacheFactory("mobius", transactionManager, null, generator, Thread.currentThread().getContextClassLoader(), new CacheDataExecutorService(configuration, generator, Thread.currentThread().getContextClassLoader()), 1000, true, true, false, 1, true);
		cluster=factory.getCluster("testr", false);
		cluster.startCluster(true);		
		
		factory2=new InfinispanCacheFactory("mobius", transactionManager, null, generator, Thread.currentThread().getContextClassLoader(), new CacheDataExecutorService(configuration, generator, Thread.currentThread().getContextClassLoader()), 1000, true, true, false, 1, true);
